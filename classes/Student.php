...
class Student {
    ...
    private $_db, $_id, $_scholar_no, $_mobile, $_parents_mobile, $_firstname, $_lastname, $_middlename, $_gender, $_category, $_email, $_department, $_programme, $_courses,$_courses_load,$_spi, $_session, $_semester,$_total_score,$_max_score, $_cpi, $_home_address, $_hostel_address,$_payment, $_blocked, $_biography,$_cv_link, $_published,$_approved;

    public function getFirstName() {
        return ($this -> _firstname != NULL) ? $this -> _firstname : NULL;
    }
    
    public function getLastName() {
        return ($this -> _lastname != NULL) ? $this -> _lastname : NULL;
    }

    public function getMiddleName() {
        return ($this -> _middlename != NULL) ? $this -> _middlename : NULL;
    }
    ...
    public function register($email,$firstname,$lastname,$middlename,$gender,$scholar_no,$category,$programme,$semester,$session,$department,$mobile,$parents_mobile,$courses,$courses_load,$home_address,$hostel_address){
        if(loggedIn()){
            $this->_connect();
            
            $email = $this -> _db -> real_escape_string(escape($email));
            $firstname = $this -> _db -> real_escape_string(escape($firstname));
            $lastname = $this -> _db -> real_escape_string(escape($lastname));
            $middlename = $this -> _db -> real_escape_string(escape($middlename));
            ...
            if($this->checkSN($scholar_no)){
                ...
                $query = "INSERT INTO students_info (email,firstname,lastname,middlename,gender,scholar_no,category,programme,semester,session,department,mobile,mobile_verified,parents_mobile,courses,courses_load,home_address,hostel_address,approved,blocked)
                          VALUES ('".$email."',
                                  '".$firstname."',
                                  '".$lastname."',
                                  '".$middlename."',
